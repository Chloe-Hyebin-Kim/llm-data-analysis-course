...
프로젝트에 필요한 설정은 공유하되, API Key, 비밀번호, 접근 토큰과 같은 민감한 정보가 Git 저장소에 노출되는 것을 방지하기 위함.
예를 들어 .env 파일에 실제 API Key를 저장하고 .gitignore에 등록하면 해당 값이 Git에 커밋되는 것을 방지할 수 있다. 
대신 .env.example에는 실제 비밀값을 제외하고 필요한 환경변수의 이름과 형식만 작성하여 다른 개발자가 프로젝트에 어떤 설정이 필요한지 알 수 있도록 한다.
